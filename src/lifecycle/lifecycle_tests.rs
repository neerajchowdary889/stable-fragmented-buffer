...
use crate::backend::shared::{SharedBackend, CHUNK_HEADER_SIZE};
use crate::lifecycle::{BlobStoreLifecycleExt, LifecycleManager};
...
fn test_ack_cleanup_frees_chunks() {
    ...
    println!(
        "[1] Created backend ns={}, chunk_size={} ({}B header + {}B data)",
        ns,
        chunk_size,
        CHUNK_HEADER_SIZE,
    ...
    println!(
        "[2] Appended 100B -> chunk {}, offset {}",
        h0.page_id, h0.offset
    );
    let h1 = backend.append(&[0xBBu8; 100]).unwrap();
    println!(
        "[3] Appended 100B -> chunk {}, offset {} (overflowed because 100 > 28B remaining)",
        h1.page_id, h1.offset
    );
    let h2 = backend.append(&[0xCCu8; 100]).unwrap();
    println!(
        "[4] Appended 100B -> chunk {}, offset {}",
        h2.page_id, h2.offset
    );
    ...
    println!(
        "[7] cleanup_chunks(ttl=30s, decay=0ms) -> freed {} chunks",
        freed
    );
    println!(
        "[8] Chunks remaining: {} (was {})",
        after_count, initial_count
    );
    println!(
        "[9] Active write chunk is never freed, so {} chunk(s) survived",
        after_count
    );
...
fn test_partial_ack_does_not_free() {
    ...
    let backend = SharedBackend::create(&ns, chunk_size, None).unwrap();
    println!(
        "[1] Created backend, data capacity = {}B",
        chunk_size - CHUNK_HEADER_SIZE
    );

    let h1 = backend.append(&[0xAAu8; 100]).unwrap();
    let _h2 = backend.append(&[0xBBu8; 100]).unwrap();
    println!(
        "[2] Wrote 2 entries to chunk {} (100B + 100B = 200B / {}B capacity)",
        h1.page_id,
        chunk_size - CHUNK_HEADER_SIZE
    );
...
fn test_decay_timeout_prevents_premature_free() {
    ...
    println!(
        "[1] Wrote to chunk {} and overflowed to next chunk",
        h0.page_id
    );
    ...
    println!(
        "[3] cleanup_chunks(decay=60s) -> freed {} (should be 0, decay not elapsed)",
        freed
    );
    assert_eq!(freed, 0);
    ...
    let freed = backend.cleanup_chunks(30_000, 0);
    println!(
        "[4] cleanup_chunks(decay=0ms) -> freed {} (should be >=1, immediate decay)",
        freed
    );
    assert!(freed >= 1);
...
fn test_ttl_expiry_frees_unacked_chunks() {
    ...
    let h1 = backend.append(&[0xBBu8; 100]).unwrap();
    println!(
        "[1] Wrote to chunk {} and chunk {} — NOT acknowledging (simulating consumer crash)",
        h0.page_id, h1.page_id
    );
    ...
    println!(
        "[3] cleanup_chunks(ttl=1ms, decay=0ms) -> freed {} chunks",
        freed
    );
...
fn test_ttl_does_not_expire_fresh_data() {
    ...
    println!(
        "[2] cleanup_chunks(ttl=30s) -> freed {} (should be 0, data is < 30s old)",
        freed
    );
...
fn test_ttl_expiry_invalidates_handles() {
    ...
    println!(
        "[3] resolve(h0) -> {:?} (should be None — chunk was unlinked)",
...
fn test_prefetch_allocates_next_chunk() {
    ...
    println!(
        "[2] Appended 85B -> chunk {} (usage = {:.0}% > 80% threshold)",
        h.page_id, usage_pct
    );

    let count = backend.chunk_count();
    println!(
        "[3] chunk_count = {} (should be >= 2 — prefetch triggered!)",
        count
    );
...
fn test_prefetch_does_not_trigger_below_threshold() {
    ...
    println!(
        "[2] Appended 100B -> chunk {} (usage = {:.0}% < 80% threshold)",
        h.page_id, usage_pct
    );

    let count = backend.chunk_count();
    println!(
        "[3] chunk_count = {} (should be 1 — no prefetch triggered)",
        count
    );
...
fn test_prefetch_reduces_overflow_latency() {
    ...
    println!(
        "[1] Wrote 85/100B -> prefetch fired -> chunk_count = {}",
        count_after_prefetch
    );

    let h2 = backend.append(&[0xBBu8; 50]).unwrap();
    let count_after_overflow = backend.chunk_count();
    println!(
        "[2] Wrote 50B -> overflowed to chunk {} -> chunk_count = {}",
        h2.page_id, count_after_overflow
    );
...
fn test_write_after_cleanup_uses_new_chunks() {
    ...
    println!(
        "[1] Filled chunks {}, {}, {}",
        h0.page_id, h1.page_id, h2.page_id
    );
...
fn test_lifecycle_manager_single_cycle() {
    ...
    let store =
        Arc::new(PinnedBlobStore::new_shared(config, &ns, CHUNK_HEADER_SIZE + 128).unwrap());
    ...
    println!(
        "[4] maintenance_cycle() -> freed {} (calls cleanup_acknowledged + cleanup_shared)",
        freed
    );
...
fn test_lifecycle_manager_background_thread() {
    ...
    println!(
        "[4] resolve(h0) -> {:?} (should be None — background thread cleaned it)",
        result.as_ref().map(|v| v.len())
    );
...
fn test_lifecycle_ttl_expiry_via_manager() {
    ...
    let freed = manager.maintenance_cycle();
    println!(
        "[3] maintenance_cycle() -> freed {} chunks via TTL expiry",
        freed
    );
...
fn test_concurrent_append_during_cleanup() {
    ...
    let store =
        Arc::new(PinnedBlobStore::new_shared(config, &ns, CHUNK_HEADER_SIZE + 256).unwrap());
    ...
    for t in 0..4 {
        ...
        writers.push(thread::spawn(move || {
            ...
            for i in 0..50 {
                ...
                let h = s
                    .append_shared(data.as_bytes())
                    .expect("append during cleanup");
                handles.push((h, data));
                if i % 2 == 0 {
    ...
    println!(
        "[4] Resolve check: {} still alive, {} already cleaned up",
        resolved_count, cleaned_count
    );
...
fn test_cleanup_does_not_corrupt_active_chunk() {
    ...
    let h2 = backend.append(&[0xBBu8; 50]).unwrap();
    println!(
        "[1] Wrote 2 entries to chunk {} (active write chunk, no overflow)",
        h1.page_id
    );
    ...
    println!(
        "    Even though all entries are acked, chunk {} is the active write_head",
        h1.page_id
    );
    ...
    println!(
        "[4] Wrote and resolved new entry on chunk {} -> {} bytes OK",
        h3.page_id,
        data.len()
    );
